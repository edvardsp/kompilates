...
static const uint32_t *crc_table = (const uint32_t *)crc32_ieee802_3;
 ...
 */
int
tlhash_init ( pTlhash tab, size_t n_buckets )
{
    size_t i;
...
tlhash_finalize ( pTlhash tab )
{
    ...
    {
        pTlhash_elem element = tab->buckets[i], next;
        while ( element != NULL )
        {
            next = element->next;
            free ( element->key );
...
int
tlhash_insert (
    pTlhash tab, void *key, size_t key_length, void *value
)
{
    void *test_entry;
...
)
{
    uint32_t hash = crc32 ( key, key_length );
    size_t bucket = hash % tab->n_buckets;
    pTlhash_elem el = tab->buckets[bucket];
 ...
 */
int
tlhash_remove ( pTlhash tab, void *key, size_t key_length )
{
    uint32_t hash = crc32 ( key, key_length );
    size_t bucket = hash % tab->n_buckets;
    pTlhash_elem el = tab->buckets[bucket], prev = NULL;
...
void
tlhash_keys ( pTlhash tab, void **keys )
{
    size_t b, i = 0;
    for ( b=0; b<tab->n_buckets; b++ )
    {
        pTlhash_elem el = tab->buckets[b];
        while ( el != NULL )
        {
            keys[i] = el->key;
            i += 1;
...
void
tlhash_values ( pTlhash tab, void **values )
{
    size_t b, i = 0;
    for ( b=0; b<tab->n_buckets; b++ )
    {
