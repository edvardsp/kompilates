...
relation : expression '=' expression { node_reduce(RELATION, NULL, 2); }
         | expression '<' expression { node_reduce(RELATION, NULL, 2); }
         | expression '>' expression { node_reduce(RELATION, NULL, 2); }
         ; 

expression : expression '+' expression        { char *buf = strdup("+"); 
                                                node_reduce(EXPRESSION, buf, 2);  }
           | expression '-' expression        { char *buf = strdup("-");
                                                node_reduce(EXPRESSION, buf, 2);  }
           | expression '*' expression        { char *buf = strdup("*");
                                                node_reduce(EXPRESSION, buf, 2);  }
           | expression '/' expression        { char *buf = strdup("/");
                                                node_reduce(EXPRESSION, buf, 2);  }
           | '-' expression %prec UMINUS      { char *buf = strdup("-");
